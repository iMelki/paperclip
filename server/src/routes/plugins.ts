 ...
 * | GET | /plugins/:pluginId/companies/:companyId/trusted-loopback-http | Read exact local HTTP rules |
 * | PUT | /plugins/:pluginId/companies/:companyId/trusted-loopback-http | Replace exact local HTTP rules |
...
export function pluginRoutes(
  ...
  async function logPluginMutationActivity(
    ...
    const companyIds = scopeCompanyId
      ? [scopeCompanyId]
   ...
   *
   * Query params:
   * - companyId: Required company scope for returned log rows
  ...
  router.get("/plugins/:pluginId/logs", async (req, res) => {
    ...
    const conditions = [
      eq(pluginLogs.pluginId, plugin.id),
      eq(pluginLogs.companyId, companyId),
    ];
    if (level) {
   ...
   *
   * Query params:
   * - `companyId` (required): Exact company scope for returned runs
  ...
  router.get("/plugins/:pluginId/jobs/:jobId/runs", async (req, res) => {
    ...
    try {
      const runs = await jobDeps.jobStore.listRunsByJob(jobId, companyId, limit);
      res.json(runs);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      res.status(500).json({ error: message });
   ...
   * - 409 if zero or multiple company config rows make tenant scope unresolved
  ...
  router.post("/plugins/:pluginId/webhooks/:endpointKey", async (req, res) => {
    ...
    // Step 5: Resolve the one configured company. This public route cannot
    // trust a caller-supplied header/query tenant selector. Zero or multiple
    // config rows are ambiguous and fail before a delivery row is created.
    const configRows = await registry.listConfigs(plugin.id);
    if (configRows.length === 0) {
      res.status(409).json({
        code: "plugin_webhook_company_unconfigured",
        error: "Plugin webhook requires exactly one configured company",
      });
      return;
    }
    if (configRows.length > 1) {
      res.status(409).json({
        code: "plugin_webhook_company_ambiguous",
        error: "Plugin webhook cannot resolve a unique configured company",
      });
      return;
    }
    const companyId = configRows[0]!.companyId;
    ...
    // Step 7: Record the company-scoped delivery in the database
    const startedAt = new Date();
    const delivery = await registry.createWebhookDelivery(
      plugin.id,
      endpointKey,
      companyId,
      {
    ...
    // Step 8: Dispatch to the worker via handleWebhook RPC. companyId is
    // top-level so the worker manager establishes the invocation scope before
    // any nested config/secret/state/HTTP host calls are admitted.
    try {
      ...
      await registry.updateWebhookDelivery(delivery.id, {
        status: "success",
        durationMs,
        finishedAt,
      });
  ...
  // ===========================================================================
  // Company-scoped host-owned trusted loopback HTTP policy
  // ===========================================================================

  router.get("/plugins/:pluginId/companies/:companyId/trusted-loopback-http", async (req, res) => {
    assertInstanceAdmin(req);
    ...
    const rules = getTrustedLoopbackHttpRules(settings?.settingsJson);
    res.json({ pluginId: plugin.id, companyId, rules });
  });

  router.put("/plugins/:pluginId/companies/:companyId/trusted-loopback-http", async (req, res) => {
    assertInstanceAdmin(req);
    const { pluginId, companyId } = req.params;
    assertCompanyAccess(req, companyId);
    ...
    try {
      const rules = parseTrustedLoopbackHttpRules(
        (req.body as { rules?: unknown } | undefined)?.rules,
      );
      await registry.setCompanyTrustedLoopbackHttpRules(plugin.id, companyId, rules);
      await logPluginMutationActivity(req, "plugin.trusted_loopback_http.updated", plugin.id, {
        pluginId: plugin.id,
        pluginKey: plugin.pluginKey,
        companyId,
      ...
      if (error instanceof TrustedLoopbackHttpPolicyError) {
  ...
  router.get("/plugins/:pluginId/dashboard", async (req, res) => {
    ...
    if (jobDeps) {
      try {
        const runs = await jobDeps.jobStore.listRunsByPlugin(
          plugin.id,
          companyId,
          undefined,
          10,
    ...
    try {
      const deliveries = await registry.listWebhookDeliveries(plugin.id, companyId, 10);
