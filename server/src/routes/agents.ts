...
export function agentRoutes(
  ...
  function computeAdapterConfigRevision(adapterConfig: unknown): string {
    return createHash("sha256").update(JSON.stringify(adapterConfig ?? {})).digest("hex");
  }
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    const expectedRevision =
      ...
      (typeof patchData.expectedAdapterConfigRevision === "string"
        ? patchData.expectedAdapterConfigRevision
        : null) ??
