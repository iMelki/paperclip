...
function installStubs(ids: { uid: number; gid: number; nodeUid?: number; nodeGid?: number }) {
  const shellLogFile = toShellPath(logFile);
  writeStub(
    "id",
    [
  ...
  for (const cmd of ["usermod", "groupmod", "chown"]) {
    writeStub(cmd, `echo "${cmd} $*" >> "${shellLogFile}"`);
  }
  writeStub("gosu", `echo "gosu $*" >> "${shellLogFile}"\nshift\nexec "$@"`);
}

function resolveEntrypointTestShell() {
  if (process.platform === "win32") {
    for (const root of [
      process.env.ProgramFiles ?? "C:\\Program Files",
      process.env["ProgramFiles(x86)"],
    ].filter((value): value is string => Boolean(value))) {
      const candidate = join(root, "Git", "usr", "bin", "sh.exe");
      if (existsSync(candidate)) return candidate;
    }
  }
  return resolveTestShellCommand("sh");
}

async function runEntrypoint(env: Record<string, string> = {}) {
  const result = await execFileAsync(resolveEntrypointTestShell(), [ENTRYPOINT, "echo", "ENTRYPOINT-CMD-RAN"], {
    env: {
      PATH: `${toShellPath(stubDir)}:/usr/bin:/bin`,
      ...env,
    },
  });
