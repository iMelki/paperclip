      ...
      return runChildProcess(
        ...
        (input.args ?? []).map((arg) => mapValue(arg, shellCommand)),
        {
          ...
          env: Object.fromEntries(
            Object.entries(input.env ?? {}).map(([key, value]) => [key, mapValue(value, false)]),
          ),
          stdin: input.stdin,
...
describe("claude_local environment diagnostics", () => {
  ...
  it("uses the managed Claude config seed for sandbox hello probes", async () => {
    ...
    const sandboxWorkspace = "/paperclip-test/managed-config";
...
if (!path.normalize(configDir).includes(path.join(".paperclip-runtime", "claude", "config"))) {
...
`, "utf8");
      ...
      const result = await testEnvironment({
        ...
        executionTarget: {
          ...
          runner: createLocalSandboxRunner({
            remoteRoot: sandboxWorkspace,
            localRoot: remoteWorkspace,
          }),
        },
  ...
  it("warns and omits --effort for sandbox probes when the installed Claude CLI does not advertise it", async () => {
    ...
    const remoteWorkspace = path.join(root, "remote-workspace");
    const sandboxWorkspace = "/paperclip-test/effort";
    const commandPath = path.join(root, "claude");
    ...
    try {
      ...
      expect(
        result.checks.some((check) => check.code === "claude_effort_flag_unsupported"),
        JSON.stringify(result.checks, null, 2),
      ).toBe(true);
      expect(
        result.checks.some((check) => check.code === "claude_hello_probe_passed"),
