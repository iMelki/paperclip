...
describeEmbeddedPostgres("plugin tenant isolation (company_id FK)", () => {
  ...
  it("pluginRegistryService.createJobRun + createWebhookDelivery persist companyId so cascade delete reaps them", async () => {
    ...
    expect(
      (await registry.listWebhookDeliveries(pluginId, companyA)).map((row) => row.id),
    ).toEqual([whA!.id]);
    expect(
      (await registry.listWebhookDeliveries(pluginId, companyB)).map((row) => row.id),
    ).toEqual([whB!.id]);
  ...
  it("pluginJobStore.listRunsByPlugin never returns another company's errors or instance runs", async () => {
    const pluginId = await seedPlugin();
    const companyA = await seedCompany();
    const companyB = await seedCompany();
    const jobId = randomUUID();
    ...
    await db.insert(pluginJobRuns).values([
      ...
      },
      {
        ...
        trigger: "scheduled",
        status: "failed",
        error: "instance-private-error",
      },
    ]);

    const store = pluginJobStore(db);
    const companyARuns = await store.listRunsByPlugin(pluginId, companyA);
    const companyBRuns = await store.listRunsByPlugin(pluginId, companyB);
    const instanceRuns = await store.listRunsByPlugin(pluginId, null);
    const companyAJobRuns = await store.listRunsByJob(jobId, companyA);
    const companyBJobRuns = await store.listRunsByJob(jobId, companyB);
    const instanceJobRuns = await store.listRunsByJob(jobId, null);

    expect(companyARuns.map((run) => [run.companyId, run.error])).toEqual([
      [companyA, "company-a-private-error"],
    ]);
    expect(companyBRuns.map((run) => [run.companyId, run.error])).toEqual([
      [companyB, "company-b-private-error"],
    ]);
    expect(instanceRuns.map((run) => [run.companyId, run.error])).toEqual([
      [null, "instance-private-error"],
    ]);
    expect(companyAJobRuns.map((run) => [run.companyId, run.error])).toEqual([
      [companyA, "company-a-private-error"],
    ]);
    expect(companyBJobRuns.map((run) => [run.companyId, run.error])).toEqual([
      [companyB, "company-b-private-error"],
    ]);
    expect(instanceJobRuns.map((run) => [run.companyId, run.error])).toEqual([
      [null, "instance-private-error"],
    ]);
  });

  it("plugin log and job-run routes require an authorized company and return only that tenant", async () => {
    const pluginId = await seedPlugin();
    const companyA = await seedCompany();
    const companyB = await seedCompany();
    const jobId = randomUUID();
    ...
    await db.insert(pluginJobRuns).values([
      ...
      },
      {
        ...
        trigger: "scheduled",
        status: "failed",
        error: "route-instance-error",
      },
    ]);
    await db.insert(pluginLogs).values([
      { pluginId, companyId: companyA, level: "error", message: "route-company-a-log" },
      { pluginId, companyId: companyB, level: "error", message: "route-company-b-log" },
      { pluginId, companyId: null, level: "error", message: "route-instance-log" },
    ]);
    ...
    expect((await request(app).get(`/api/plugins/${pluginId}/logs`)).status).toBe(400);
    expect(
      (await request(app).get(`/api/plugins/${pluginId}/logs?companyId=${companyB}`)).status,
    ).toBe(403);
    const logs = await request(app)
      .get(`/api/plugins/${pluginId}/logs?companyId=${companyA}`);
    expect(logs.status).toBe(200);
    expect(logs.body.map((row: { companyId: string | null; message: string }) => [
      row.companyId,
      row.message,
    ])).toEqual([[companyA, "route-company-a-log"]]);

    expect(
      (await request(app).get(`/api/plugins/${pluginId}/jobs/${jobId}/runs`)).status,
    ).toBe(400);
    expect(
      (await request(app).get(
        `/api/plugins/${pluginId}/jobs/${jobId}/runs?companyId=${companyB}`,
      )).status,
    ).toBe(403);
    const runs = await request(app)
      .get(`/api/plugins/${pluginId}/jobs/${jobId}/runs?companyId=${companyA}`);
    expect(runs.status).toBe(200);
    expect(runs.body.map((row: { companyId: string | null; error: string | null }) => [
      row.companyId,
      row.error,
    ])).toEqual([[companyA, "route-company-a-error"]]);
  });
