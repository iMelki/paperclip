...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  it("rejects branch reconciliation when runtime service activation is already spawning", async () => {
    ...
    const runtimeServiceScript = path.join(os.tmpdir(), `paperclip-runtime-service-${randomUUID()}.cjs`);
    ...
    await fs.writeFile(runtimeServiceScript, serverScript, "utf8");
    const command = `${shellQuotePath(process.execPath)} ${shellQuotePath(runtimeServiceScript)}`;
    ...
    try {
      ...
      await fs.rm(runtimeServiceScript, { force: true });
    }
