...
describe("plugin proactive http.fetch company scope", () => {
  function makeHttpHandle(seededCompanies: readonly string[]) {
  ...
  it("admits an explicit configured company on a proactive HTTP callback", async () => {
    const { handle, fetch } = makeHttpHandle(["company-1"]);
    try {
      await handle.start();
      await handle.call("getData", {
        params: {
          mode: "omit",
          hostMethod: "http.fetch",
          requestedCompanyId: "company-1",
        },
      } as unknown as HostToWorkerMethods["getData"][0]);
  ...
  it("rejects a proactive HTTP callback outside configured company scopes", async () => {
    const { handle, fetch } = makeHttpHandle(["company-1"]);
    try {
      await handle.start();
      await expect(handle.call("getData", {
