...
describe("openapi routes", () => {
  ...
  it("documents auth and reviewed response-code invariants", () => {
    ...
    expect(
      Object.keys(spec.paths["/api/plugins/{pluginId}/webhooks/{endpointKey}"].post.responses)
        .sort(),
    ).toEqual(["200", "400", "404", "409", "501", "502"]);
    expect(
      spec.paths["/api/plugins/{pluginId}/webhooks/{endpointKey}"].post.security,
    ).toEqual([]);
    expect(spec.paths["/api/plugins/{pluginId}/dashboard"].get.responses["404"]).toBeDefined();
    expect(
      spec.paths["/api/plugins/{pluginId}/logs"].get.parameters,
    ).toEqual(expect.arrayContaining([
      expect.objectContaining({ in: "query", name: "companyId", required: true }),
    ]));
    expect(
      spec.paths["/api/plugins/{pluginId}/jobs/{jobId}/runs"].get.parameters,
    ).toEqual(expect.arrayContaining([
      expect.objectContaining({ in: "query", name: "companyId", required: true }),
    ]));
    expect(
      spec.paths[
        "/api/plugins/{pluginId}/companies/{companyId}/trusted-loopback-http"
      ].put.requestBody.content["application/json"].schema.properties.rules,
    ).toMatchObject({
      type: "array",
      maxItems: 100,
    });
