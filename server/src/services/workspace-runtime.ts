...
async function spawnLocalRuntimeService(input: StartLocalRuntimeServiceInput): Promise<LocalRuntimeServiceStart> {
  ...
  if (adoptedRecord) {
    ...
    if (!(await isRuntimeServiceUrlHealthy(adoptedUrl, {
      ...
      const termination = await terminateLocalService(adoptedRecord, {
        expectedStartedAt: adoptedRecord.startedAt,
      });
      if (!termination.confirmedStopped) {
        throw new Error(`Runtime service termination unconfirmed: ${termination.outcome}`);
      }
  ...
  const readinessPromise = Promise.race([
    ...
    if (child.pid) {
      const termination = await terminateLocalService({
        ...
        processGroupId: child.pid,
        port,
      }, { trustedPid: child.exitCode === null && child.signalCode === null });
      if (!termination.confirmedStopped) {
        throw new Error(`Runtime service termination unconfirmed: ${termination.outcome}`);
      }
    }
...
async function stopRuntimeService(serviceId: string) {
  ...
  if (record.child && record.child.pid) {
    const termination = await terminateLocalService({
      ...
      port: record.port,
    }, { trustedPid: record.child.exitCode === null && record.child.signalCode === null });
    if (!termination.confirmedStopped) {
      throw new Error(`Runtime service termination unconfirmed: ${termination.outcome}`);
    }
  } else if (record.providerRef) {
    const pid = Number(record.providerRef);
    if (!Number.isSafeInteger(pid) || pid <= 0) {
      throw new Error("Runtime service termination unconfirmed: invalid_pid");
    }
    {
      const termination = await terminateLocalService({
        pid,
