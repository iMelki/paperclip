...
async function execLocalPluginCommand(
  ...
  // pnpm is installed as a .cmd launcher on Windows. Node's execFile/spawn
  // cannot execute batch files directly (it fails with EINVAL/EFTYPE), so use
  // the system cmd.exe while preserving argv boundaries.
...
export async function ensureLocalPluginBuilt(
  ...
  const runExecFileAsync = options.execFileAsyncImpl ?? execLocalPluginCommand;
