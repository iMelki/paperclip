 ...
 * authoritative gate — see the note in runRelatedSuites about where exhaustiveness actually
 * lands. Correctness gates (forbidden tokens, gitleaks, react-doctor, typecheck) are untouched
 * by this cap.
 *
...
async function runRelatedSuites(files) {
  ...
  console.log(
    `\n[test:run] ${selected.length} suites import the staged files (cap ${cap}). ` +
      ...
      `Exhaustive verification lands at the dev -> master PR: .github/workflows/pr.yml is scoped ` +
      `to 'pull_request: branches: [master]' and runs the full sharded general + serialized ` +
      `suites there. NOTE: nothing runs the suite on PRs into dev or on pushes to dev, so a ` +
      `regression can sit on dev until that promotion PR.\n` +
