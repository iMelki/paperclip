...
describe("routine and plugin parity commands", () => {
  ...
  it("wraps deeper plugin endpoints", async () => {
    ...
    await run(["plugin", "logs", "plug", "--company-id", COMPANY_ID]);
    ...
    await run(["plugin", "job:runs", "plug", "job1", "--company-id", COMPANY_ID]);
    ...
    expect(fetchMock.mock.calls.map((call) => [call[1]?.method ?? "GET", call[0]])).toEqual([
      ...
      ["GET", `http://localhost:3100/api/plugins/plug/logs?companyId=${COMPANY_ID}`],
      ...
      ["GET", `http://localhost:3100/api/plugins/plug/jobs/job1/runs?companyId=${COMPANY_ID}`],
