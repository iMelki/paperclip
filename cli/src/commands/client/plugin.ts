...
export function registerPluginCommands(program: Command): void {
  ...
  addPluginCompanySubGet(plugin, "logs", "Get company-scoped plugin logs", "logs");
  ...
  addPluginCompanySubGet(plugin, "dashboard", "Get company-scoped plugin dashboard data", "dashboard");
...
function addPluginCompanySubGet(
  parent: Command,
  name: string,
  description: string,
  suffix: string,
    ...
    parent
      ...
      .action(async (pluginId: string, opts: PluginCompanyOptions) => {
        try {
          ...
          printOutput(
            await ctx.api.get(
              `/api/plugins/${encodeURIComponent(pluginId)}/${suffix}?companyId=${encodeURIComponent(companyId)}`,
            ),
            { json: ctx.json },
          );
        } catch (err) {
...
function addPluginJobGet(parent: Command, name: string, description: string, suffix: string): void {
  addCommonClientOptions(
    parent
      ...
      .requiredOption("-C, --company-id <id>", "Company ID")
      .action(async (pluginId: string, jobId: string, opts: PluginCompanyOptions) => {
        try {
          const ctx = resolveCommandContext(opts, { requireCompany: true });
          const companyId = requireCompanyId(ctx);
          printOutput(
            await ctx.api.get(
              `/api/plugins/${encodeURIComponent(pluginId)}/jobs/${encodeURIComponent(jobId)}/${suffix}?companyId=${encodeURIComponent(companyId)}`,
            ),
            { json: ctx.json },
          );
        } catch (err) {
