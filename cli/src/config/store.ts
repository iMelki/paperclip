...
function durableCopyFile(sourcePath: string, destinationPath: string, flags = 0): void {
  ...
  try {
    try {
      ...
      const code = error instanceof Error && "code" in error ? error.code : null;
      // Windows can reject fsync on a freshly copied read-only descriptor even
      // though the copy and ACL are durable enough for this recovery backup.
      // Keep the fail-closed behavior on other platforms and unexpected errors.
      if (process.platform !== "win32" || !["EACCES", "EINVAL", "ENOTSUP", "EPERM"].includes(String(code))) {
        throw error;
      }
    }
  } finally {
