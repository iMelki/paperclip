...
- the Process adapter treats that resolved workspace as authoritative and exposes a secret-free `paperclip.process-context.v1` object in `PAPERCLIP_VALIDATOR_CONTEXT_JSON`, alongside stable `PAPERCLIP_RUN_ID`, `PAPERCLIP_ISSUE_*`, `PAPERCLIP_PROJECT_ID`, and execution-workspace environment variables for deterministic validators
