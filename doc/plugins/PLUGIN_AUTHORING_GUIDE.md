...
### When a plugin must call a loopback service

Use `ctx.http.fetch` and keep `http.outbound` in the manifest. Loopback remains
blocked unless an instance administrator with company access creates a
host-owned exact rule for this plugin/company. The rule must name:

- the exact method;
- `http://127.0.0.1:<port>` or `http://[::1]:<port>` with an explicit port;
- an exact path, optionally with one whole-segment `*`.

Inside an event, webhook, scoped API route, tool call, or UI bridge invocation,
the host derives the company automatically. A scheduled reconciliation loop or
other proactive callback must make the company explicit:

```ts
await ctx.http.fetch(
  "http://127.0.0.1:3021/api/callbacks/run-123",
  {
    method: "POST",
...
Do not use `localhost`, credentials, query strings, fragments, encoded
separators/dots, dot segments, HTTPS loopback, or a missing port. Non-matching
requests still pass through the standard SSRF guard and loopback stays denied.
Plugins cannot create or change these rules themselves; operators manage the
reserved policy through the Paperclip API.
