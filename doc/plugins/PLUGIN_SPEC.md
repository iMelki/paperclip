...
#### Company-scoped trusted loopback HTTP

`http.outbound` continues to deny loopback and private/reserved destinations by
default. An instance administrator who also has access to the target company
may opt one plugin/company into narrowly defined local HTTP calls through:

- `GET /api/plugins/:pluginId/companies/:companyId/trusted-loopback-http`
- `PUT /api/plugins/:pluginId/companies/:companyId/trusted-loopback-http`

Rules are host-owned settings under
`plugin_company_settings.settings_json.__paperclipHost.trustedLoopbackHttpRules`.
Each rule has an exact HTTP `method`, a literal origin of
`http://127.0.0.1:<explicit-port>` or `http://[::1]:<explicit-port>`, and a
`pathnamePattern`. A path pattern may contain at most one `*`, and it must be a
whole path segment that matches exactly one non-empty segment. Credentials,
query strings, fragments, encoded wildcards/dots/path separators, dot segments,
DNS aliases such as `localhost`, HTTPS loopback, and implicit ports are
rejected.

Plugins cannot write the reserved host namespace. Host-policy updates and
plugin/local-folder settings writes serialize on the company-settings row and
merge the live reserved subtree, so a stale plugin write cannot resurrect a
revoked rule. A call made inside a company-scoped host invocation inherits that
company. A proactive worker call must pass `{ companyId }` as the third
argument to `ctx.http.fetch`; the host admits it only when that company is in
the worker's configured proactive scope. If no exact rule matches, the existing
DNS-pinned SSRF guard runs unchanged.

Plugin dashboard, log, webhook-delivery, and job-run diagnostics require an
authorized `companyId` and query only that exact company scope. Public webhook
callers cannot select a tenant: the host accepts delivery only when the plugin
has exactly one configured company, then supplies that company to the worker.
...
2. The public caller cannot select a company by header, query parameter, or
   body. The host resolves company scope from persisted plugin configuration.
3. Exactly one configured company is required. Zero rows return HTTP `409`
   with `plugin_webhook_company_unconfigured`; more than one returns HTTP `409`
   with `plugin_webhook_company_ambiguous`. Both fail before delivery creation.
4. The worker receives `companyId` plus the exact raw request bytes, parsed body,
   headers, endpoint key, and request id through `handleWebhook`.
5. The host records the delivery with that same `companyId`.
6. Signature verification happens in plugin code using secret refs resolved by the host.
7. Every delivery is recorded.
8. Webhook handling must be idempotent.
9. Operator dashboard queries require an authorized `companyId` and return only
   that company's webhook deliveries.
