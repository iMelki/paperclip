...
`--no-run` keeps non-interactive onboarding headless: it writes the resolved
configuration and local key material without starting the foreground server.
Without it, `--yes` retains the quickstart behavior of starting immediately.
