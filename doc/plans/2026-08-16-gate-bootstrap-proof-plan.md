...
Focused broken/restored receipts and independent final review are complete.
Post-merge-base validation and hosted PR readback remain pending.
The first workflow change cannot prove its own `dev` trigger until the bootstrap
change is present on the base branch; refresh a dependent PR after that merge.
...
| `scripts/git-push-scan-integrity.mjs` | new | 98 | Tracked-manifest normalization, scoped/repository-wide hidden-index rejection, destination enumeration, and missing-path reconciliation. |
| `scripts/git-push-scan-integrity.test.mjs` | new | 66 | Enumeration, scoped/repository-wide index-state, case-semantics, and missing-manifest fixtures under the selector-required sibling name. |
| `scripts/check-no-git-push-source.test.mjs` | new | 321 | Adversarial language/command/marker fixtures, including executable multiline data, column-zero here-string closure, scan-visible quoted/unquoted and continued LF/CRLF heredoc bodies and terminators, modern, legacy, and indexed-array arithmetic shifts, nested command substitutions, quote removal, and batch continuations. |
| `scripts/check-no-git-push.test.mjs` | 416 | 688 | Filesystem and tracked-manifest integration matrix; helper-level assertions moved to their selector-required sibling suite. |
| `scripts/pre-push-check.ps1` | 248 | 213 | Thin Git-private logging/orchestration caller. |
| `scripts/pre-push-check.sh` | 145 | 141 | POSIX mirror of the same caller contract. |
| `scripts/run-vitest-stable.mjs` | 695 | 748 | Existing runner plus exact-file mode; reviewed exception required. |
| `scripts/run-vitest-direct.mjs` | new | 31 | Shell-free canonical Vitest launcher. |
...
| `scripts/check-pr-workflow-trigger.mjs` | new | 327 | Strict trigger/job/active-command policy checker split into cohesive validators. |
| `scripts/check-pr-workflow-trigger.test.mjs` | new | 219 | Missing-branch, YAML-key, opener, conditional, custom-shell, and masked-command bypass fixtures. |
| `scripts/is-main-module.mjs` | new | 24 | Realpath-aware fail-closed ESM entry helper. |
| `scripts/pre-push-test-selection.mjs` | new | 348 | Canonical selector with exhaustive path ownership and deletion routing. |
| `scripts/pre-push-test-selection.test.mjs` | new | 216 | Runner, runtime-asset ownership, deletion, and path-integrity fixtures. |
| `scripts/run-pre-push-tests.mjs` | new | 394 | Git-protocol/HEAD/remote planner and runner dispatcher with repository-wide normal-index binding; Git's literal `HEAD` local-ref form is accepted only with the same object binding. |
| `scripts/run-pre-push-tests.test.mjs` | new | 263 | Protocol, literal-HEAD compatibility, authority, cleanliness, hidden-index, and exit fixtures. |
| `scripts/pre-push-callers.test.mjs` | new | 200 | Static wiring plus real caller exit/argument fixtures. |
| `scripts/scan-pre-push-secrets.mjs` | new | 135 | Exact outgoing-range orchestrator. |
| `scripts/scan-pre-push-secrets.test.mjs` | new | 108 | Remote authority, deletion, and child-exit fixtures. |
...
- `selectPrePushTests` is 80 physical lines. Discovery is extracted into a
  separate 49-line helper; nesting is at most four levels.
- planner functions remain below 50 physical lines. The CLI tail is linear and
  has no nested process graph.
...
independently reviewed before merge. Exact-file parsing reuses its established
isolated Vitest process contract, while process launch was extracted into the
31-line shell-free `run-vitest-direct.mjs`. Duplicating isolation in a second
runner would increase drift and rollback risk. The scanner filesystem test is
688 lines after lexer and manifest-helper fixtures moved to their 321- and
66-line owning suites; none of the three scanner tests needs an oversized-test
exception.

PowerShell review: all changed lines in `scripts/pre-push-check.ps1` are at most
120 characters. Existing long lines in `scripts/pre-commit-check.ps1` were not
introduced or expanded by this repair. PSScriptAnalyzer remains part of focused
validation when host admission permits.
...
| exact selector | Delete a changed test/source, use an untracked sibling, add uncovered production, pass an escaping/whitespace/symlink path, and change root/runtime non-JS files. | Exact surviving Node/Vitest siblings are tracked and assigned once; deletions that cannot execute locally and every remaining non-doc path are declared hosted CI. |
| Git update planner | Empty/malformed/deletion protocol, dirty/staged/untracked worktree, repository-wide skip-worktree/assume-unchanged state, non-HEAD and multi-ref updates, mismatched remote URL, malformed remote-dev advertisement, and child exit 19. The first real `HEAD:topic` push also proved that rejecting Git's literal `HEAD` local-ref is a false lockout. | One pristine HEAD with a normal index uses the configured destination's advertised dev; both `refs/heads/...` and literal `HEAD` local-ref forms remain bound to the same checked-out object, and restored child exit 0 passes. |
| platform callers and Husky | Fake scanner exit 17, secret exit 23, and planner exit 19 through both real callers; omit the remote location; verify logs resolve under Git-private storage. | Fake children exit 0, both remote arguments reach both planners, and restored dispatch passes. |
| PR workflow policy | Remove `dev`; add plain, quoted, escaped, duplicate, or whitespace-padded event keys; comment, condition, or mask the exact secret/test commands. | Real workflow has one plain pull-request event for master/dev, no push event, one unmasked exact secret range, and active unmasked gate regressions. |
...
| shell-free Vitest launch | Pass a filename containing cmd metacharacters and capture the spawned command/argv. | Filename remains one literal argument to `process.execPath`, with `shell:false` and no side effect. |
