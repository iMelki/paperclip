...
| `fastMode` | boolean | No | Enables Codex Fast mode for supported or explicitly configured models; burns credits faster |
| `sandboxMode` | string | No | Structured Codex sandbox mode: `read-only`, `workspace-write` (new-agent default), or `danger-full-access` |
| `approvalPolicy` | string | No | Structured Codex approval policy: `untrusted`, `on-request`, or `never` (new-agent default) |
| `networkAccess` | boolean | No | Codex workspace-write sandbox network access; Paperclip `networkScope` remains the outer boundary |
| `configProfile` | string | No | Optional profile layered from the agent's curated `CODEX_HOME` |
| `ignoreUserConfig` | boolean | No | Ignore `config.toml`; defaults to `false` and is reserved for sterile canaries/diagnostics |
| `dangerouslyBypassApprovalsAndSandbox` | boolean | No | Skip approvals and sandboxing; defaults to `false` and requires explicit opt-in |
| `extraArgs` | string[] | No | Additional non-managed Codex arguments. Paperclip rejects raw model, sandbox, approval, config/profile, session, prompt, and bypass overrides. |
