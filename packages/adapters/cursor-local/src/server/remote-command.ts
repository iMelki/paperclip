...
export async function prepareCursorSandboxCommand(input: {
  ...
  // This environment is consumed by a POSIX remote sandbox, so its fallback
  // must not inherit the Paperclip host's platform-specific PATH (notably a
  // semicolon-delimited Windows PATH).
  const currentPath =
    input.env.PATH ??
    input.env.Path ??
