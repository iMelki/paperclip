  ...
  if (targetIsRemote && adapterExecutionTargetUsesManagedHome(target) && !hasExplicitClaudeConfigDir) {
    ...
    try {
      ...
      const runtimeRootDir =
        preparedRuntime.runtimeRootDir ?? joinPortablePath(managedRemoteCwd, ".paperclip-runtime", "claude");
      const remoteClaudeConfigSeedDir =
        preparedRuntime.assetDirs["config-seed"] ?? joinPortablePath(runtimeRootDir, "config-seed");
      const remoteClaudeConfigDir = joinPortablePath(runtimeRootDir, "config");
