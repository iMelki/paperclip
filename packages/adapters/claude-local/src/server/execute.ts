...
import { joinPortablePath } from "@paperclipai/adapter-utils/shell-path";
import {
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const effectivePromptBundleAddDir = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.skills ??
      joinPortablePath(effectiveExecutionCwd, ".paperclip-runtime", "claude", "skills")
    : promptBundle.addDir;
  const effectiveInstructionsFilePath = promptBundle.instructionsFilePath
    ? executionTargetIsRemote
      ? joinPortablePath(effectivePromptBundleAddDir, path.basename(promptBundle.instructionsFilePath))
  ...
  const effectiveMcpConfigPath = executionTargetIsRemote
    ? joinPortablePath(
        preparedExecutionTargetRuntime?.assetDirs["mcp-config"] ??
          joinPortablePath(effectiveExecutionCwd, ".paperclip-runtime", "claude", "mcp-config"),
        path.basename(localMcpConfigPath),
      )
    : localMcpConfigPath;
  const remoteClaudeRuntimeRoot = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.runtimeRootDir ??
      joinPortablePath(effectiveExecutionCwd, ".paperclip-runtime", "claude")
    : null;
  const remoteClaudeConfigSeedDir = claudeConfigSeedDir && remoteClaudeRuntimeRoot
    ? preparedExecutionTargetRuntime?.assetDirs["config-seed"] ??
      joinPortablePath(remoteClaudeRuntimeRoot, "config-seed")
    : null;
  const remoteClaudeConfigDir = useManagedRemoteClaudeConfig && remoteClaudeRuntimeRoot
    ? joinPortablePath(remoteClaudeRuntimeRoot, "config")
    : null;
  ...
  const retainPaperclipDependentCleanup = (error: unknown) => {
    ...
    return retainAndSealAdapterExecutionTargetPaperclipBridgeDependentResources({
      error,
      registrationId: `adapter:claude:${runId}`,
      cleanupSteps: [
        ...(restoreRemoteWorkspace
          ? [{ label: "claude-restore-remote-workspace", run: restoreRemoteWorkspace }]
          : []),
      ],
    });
  };
  if (executionTargetIsRemote && adapterExecutionTargetUsesPaperclipBridge(runtimeExecutionTarget)) {
    if (!ctx.onEvent) {
      throw new Error(
        "Remote Claude callback execution requires a durable adapter event sink before provider dispatch.",
      );
    }
    try {
