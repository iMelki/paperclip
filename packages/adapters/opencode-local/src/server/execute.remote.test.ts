...
describe("opencode remote execution", () => {
  ...
  it.each(["ssh", "sandbox"] as const)(
    ...
    async (transport) => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-opencode-denial-"));
  ...
  it.each([
    ...
    ["malformed legacy", { executionTransport: { remoteExecution: { remoteCwd: "/remote/workspace" } } }],
    ...
    const onEvent = vi.fn(async () => undefined);
    const onSpawn = vi.fn(async () => undefined);
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-opencode-invalid-"));
    cleanupDirs.push(rootDir);
    const sideEffectPath = path.join(rootDir, "must-not-exist");
  ...
  it("leaves explicit local execution unaffected by the remote callback gate", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-opencode-local-gate-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      await expect(execute({
        ...
        config: { command: "opencode", model: "openai/gpt-4.1" },
