...
describe("pi remote execution", () => {
  ...
  it.each(["ssh", "sandbox"] as const)(
    ...
    async (transport) => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-pi-denial-"));
  ...
  it.each([
    ...
    ["malformed legacy", { executionTransport: { remoteExecution: { port: 2222, username: "fixture" } } }],
    ...
    const onEvent = vi.fn(async () => undefined);
    const onSpawn = vi.fn(async () => undefined);
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-pi-invalid-"));
    cleanupDirs.push(rootDir);
    const sideEffectPath = path.join(rootDir, "must-not-exist");
  ...
  it("leaves explicit local execution unaffected by the remote callback gate", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-pi-local-gate-"));
    cleanupDirs.push(rootDir);
