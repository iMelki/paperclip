    ...
    const runAttempt = async (resumeSessionId: string | null) => {
      ...
      if (onMeta) {
        await onMeta({
          ...
          effectiveConfig: {
            model: {
              value: execArgs.model || null,
              origin: execArgs.model ? "adapter_config" : "codex_default",
            },
            sandbox: {
              mode: execArgs.sandboxMode,
              origin:
                execArgs.sandboxMode === null
                  ? "codex_home"
                  : execArgs.bypassApprovalsAndSandbox
                    ? "bypass"
                    : "adapter_config",
            },
            approval: {
              policy: execArgs.approvalPolicy,
              origin:
                execArgs.approvalPolicy === null
                  ? "codex_home"
                  : execArgs.bypassApprovalsAndSandbox
                    ? "bypass"
                    : "adapter_config",
            ...
            network: {
              codexWorkspaceAccess: execArgs.networkAccess,
              paperclipScope: networkScope ?? "host_default",
              allowlistCount: parseLocalProcessNetworkAllowlist(config.networkAllowlist).length,
            },
            timeout: {
              seconds: timeoutSec,
              origin: timeoutResolution.source,
            },
            configuration: {
              origin: execArgs.configurationOrigin,
              homeOrigin:
                configuredCodexHome == null
                  ? "paperclip_managed_company"
                  : configuredHomeIsManaged
                    ? "paperclip_managed_agent"
                    : "agent_override",
              profile: execArgs.configProfile,
              ignoreUserConfig: execArgs.ignoreUserConfig,
            },
          },
          prompt,
          promptMetrics,
