...
import {
  joinPortablePath,
  shellQuotePath,
} from "@paperclipai/adapter-utils/shell-path";
...
export function buildCodexAuthInboundProvision(): SandboxManagedRuntimeAssetProvision {
  return {
    ...
    postUploadCommand: ({ assetTarPath, assetDir, runtimeRootDir }) =>
      `sh ${shellQuotePath(joinPortablePath(runtimeRootDir, CODEX_AUTH_MERGE_EXTRACT_SCRIPT_NAME))} ` +
      `${shellQuotePath(assetDir)} ${shellQuotePath(assetTarPath)}`,
  };
}
