...
export async function testEnvironment(
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "codex")) {
      ...
      let execArgs: ReturnType<typeof buildCodexExecArgs>;
      try {
        execArgs = buildCodexExecArgs(
          { ...config, fastMode: false },
          { skipGitRepoCheck: targetIsSandbox },
        );
      } catch (error) {
        if (!(error instanceof CodexAdapterArgumentConflictError)) throw error;
        checks.push({
          code: error.code,
          level: "error",
          message: error.message,
          detail: error.conflicts.join(", "),
          hint: "Remove raw managed flags and use the structured Codex adapter fields.",
        });
        return {
