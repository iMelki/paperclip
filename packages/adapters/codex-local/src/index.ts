...
- configProfile (string, optional): optional profile layered from the curated CODEX_HOME
- ignoreUserConfig (boolean, optional): omit config.toml loading; false by default and reserved for sterile canaries/diagnostics
- dangerouslyBypassApprovalsAndSandbox (boolean, optional): run with bypass flag; false by default and explicit opt-in only
- command (string, optional): defaults to "codex"
- extraArgs (string[], optional): additional non-managed CLI args; raw model, sandbox, approval, config/profile, session, prompt, and bypass overrides are rejected
...
- Normal roles load their curated per-agent CODEX_HOME and may select a structured configProfile. Set ignoreUserConfig=true only for reproducibility canaries or diagnostics; it cannot be combined with configProfile.
