...
**Jobs:** Declare in `manifest.jobs` with `jobKey`, `displayName`, `schedule` (cron). Register handler with `ctx.jobs.register(jobKey, fn)`. **Webhooks:** Declare in `manifest.webhooks` with `endpointKey`; handle in `onWebhook(input)`. Public webhook delivery requires exactly one configured company; `input.companyId` is host-resolved and also scopes nested host calls. **State:** `ctx.state.get/set/delete(scopeKey)`; scope kinds: `instance`, `company`, `project`, `project_workspace`, `agent`, `issue`, `goal`, `run`.
...
**Trusted loopback HTTP:** `ctx.http.fetch` still blocks loopback/private
destinations by default. An instance administrator with company access can add
host-owned exact method/origin/path rules for literal
`http://127.0.0.1:<port>` or `http://[::1]:<port>`. A host-issued
company-scoped invocation inherits its company. Proactive work must pass a
third argument:

```ts
await ctx.http.fetch(url, { method: "POST", body }, { companyId });
```

Rules allow at most one whole-segment `*`, which matches exactly one non-empty
segment; credentials, query strings, fragments, `localhost`, dot segments, and
encoded wildcards/dots/path separators remain blocked. Plugins cannot write
the reserved host policy namespace, and stale plugin settings writes cannot
restore a rule that the host has revoked.

Operator diagnostics for logs, webhook deliveries, dashboard job runs, and
plugin settings are company-scoped. The REST API and CLI require an authorized
company ID for these views.
