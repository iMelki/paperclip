...
export function createHostClientHandlers(
  ...
  return {
    ...
    // HTTP
    "http.fetch": gated("http.fetch", async (params, context) => {
      const invocationCompanyId = readNonEmptyString(context?.invocationScope?.companyId);
      return services.http.fetch({
        ...params,
        companyId: params.companyId ?? invocationCompanyId ?? undefined,
      });
    }),
