...
// Git archive and worktree helper scripts run in a POSIX shell even on a
// Windows host. Normalize a drive-letter path before quoting so `C:` is not
// parsed as a remote host selector.
const shellQuote = shellQuotePath;

function isolatedGitEnv(base: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  ...
  return await new Promise<GitCommandResult>((resolve, reject) => {
    execFile(
      ...
      ["-C", localDir, ...args],
      {
        ...
        env: isolatedGitEnv(options.env ?? process.env),
      },
      (error, stdout, stderr) => {
        if (error) {
